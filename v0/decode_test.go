...
func TestDecodeFromEvent_DistributorRegistered(t *testing.T) {
	...
	payload := buildWatcherEventPayload(v0.EventDistributorRegistered.String(), data)
	event := buildEvent(t, payload)

	result, err := v0.DecodeFromEvent(context.Background(), event)
	require.NoError(t, err)

	require.Equal(t, v0.EventDistributorRegistered, result.EventName())

	concrete, ok := result.ConcreteEvent.(*v0.DistributorRegistered)
...
func TestDecodeFromEvent_DistributorRequestProcessed(t *testing.T) {
	...
	payload := buildWatcherEventPayload(v0.EventDistributorRequestProcessed.String(), data)
	event := buildEvent(t, payload)

	result, err := v0.DecodeFromEvent(context.Background(), event)
	require.NoError(t, err)

	require.Equal(t, v0.EventDistributorRequestProcessed, result.EventName())

	concrete, ok := result.ConcreteEvent.(*v0.DistributorRequestProcessed)
...
func TestDecodeFromEvent_SubscriptionRequested(t *testing.T) {
	...
	payload := buildWatcherEventPayload(v0.EventSubscriptionRequested.String(), data)
	event := buildEvent(t, payload)

	result, err := v0.DecodeFromEvent(context.Background(), event)
	require.NoError(t, err)

	require.Equal(t, v0.EventSubscriptionRequested, result.EventName())

	concrete, ok := result.ConcreteEvent.(*v0.SubscriptionRequested)
...
func TestDecodeFromEvent_ScientificNotation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			eventType:      v0.EventSubscriptionRequested.String(),
			amountValue:    "1.2e+21",
...
func TestDecodeFromEvent_UnsupportedEvent(t *testing.T) {
	...
	_, err := v0.DecodeFromEvent(context.Background(), event)
	require.Error(t, err)
...
func TestDecodeFromEvent_RedemptionRequested(t *testing.T) {
	...
	payload := buildWatcherEventPayload(v0.EventRedemptionRequested.String(), data)
	event := buildEvent(t, payload)

	result, err := v0.DecodeFromEvent(context.Background(), event)
	require.NoError(t, err)

	require.Equal(t, v0.EventRedemptionRequested, result.EventName())

	concrete, ok := result.ConcreteEvent.(*v0.RedemptionRequested)
...
func TestEventPayloadRoundTrip(t *testing.T) {
	...
	originalPayload := buildWatcherEventPayload(v0.EventDistributorRegistered.String(), data)
