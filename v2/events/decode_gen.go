...
var eventDecoders = map[EventName]EventDecoder{
	...
	EventDistributorAuthorizationUpdated:      decodeDistributorAuthorizationUpdated,
...
func decodeDistributorAuthorizationUpdated(params map[string]string, _ string) (ConcreteEvent, error) {
	authorized, err := strconv.ParseBool(params["authorized"])
	if err != nil {
		return nil, fmt.Errorf("parse authorized %q: %w", params["authorized"], err)
	}
	return &DistributorAuthorizationUpdated{
		DistributorAddr: common.HexToAddress(params["distributor_addr"]),
		FundAdminAddr:   common.HexToAddress(params["fund_admin_addr"]),
		FundTokenId:     common.HexToHash(params["fund_token_id"]),
...
func decodeRedemptionRequested(params map[string]string, _ string) (ConcreteEvent, error) {
	...
	return &RedemptionRequested{
		...
		FundTokenId:     common.HexToHash(params["fund_token_id"]),
		DistributorAddr: common.HexToAddress(params["distributor_addr"]),
		ReferenceID:     common.HexToHash(params["reference_id"]),
		RequestId:       common.HexToHash(params["request_id"]),
		Shares:          shares,
		CreatedAt:       createdAt,
	}, nil
