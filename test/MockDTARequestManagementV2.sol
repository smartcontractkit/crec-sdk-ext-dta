...
/// @title MockDTARequestManagementV2
/// @notice Minimal v2-compatible mock for testing the DTA v2 extension watcher
///         and deployed v2 certification flow.
/// @dev    The v2 ABI differs from v1 by including `referenceID` in the
///         DistributorRequest tuple returned by getDistributorRequest.
contract MockDTARequestManagementV2 {
    ...
    // --- Events (match DTARequestManagement ABI subset used by certification) ---
    ...
    // Fixed identifiers for deterministic mock data
    bytes32 public constant MOCK_FUND_TOKEN_ID = keccak256("mock-fund-token");
    bytes32 public constant MOCK_REFERENCE_ID = keccak256("mock-reference-id");
    ...
    function emitDistributorRequestProcessing() external returns (bytes32 requestId) {
        requestId = keccak256(abi.encodePacked(block.number, msg.sender, "v2"));
    ...
    /// @notice Returns hardcoded v2 DistributorRequest for any requestId.
