...
type FieldDef struct {
	...
	GoType     string
	JSONTag    string
	ParseFunc  string // Function to parse from string (empty = direct assignment)
	HasError   bool   // Whether parsing can fail
	IsEnum     bool   // Whether this is an enum type (needs cast after parsing)
	EnumType   string // The enum type name if IsEnum is true
	LocalVar   string // Local variable name for parsed value
	ValueExpr  string // Expression to use in struct literal
}
...
// getParseInfo returns parsing function, whether it has error, and value expression for a field type.
func getParseInfo(goType, jsonTag, localVar string, isEnum bool, enumType string) (parseFunc string, hasError bool, valueExpr string) {
	...
	// Handle enum types - parse as uint8 then cast
	if isEnum {
...
`
	...
	var buf bytes.Buffer
	if err := t.Execute(&buf, events); err != nil {
		fmt.Fprintf(os.Stderr, "failed to execute decoders template: %v\n", err)
		os.Exit(1)
	}

	formatted, err := format.Source(buf.Bytes())
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to format decoders code: %v\n", err)
		fmt.Fprintln(os.Stderr, "Raw code:")
		fmt.Fprintln(os.Stderr, buf.String())
		os.Exit(1)
	}

	outputPath := "v1/decode_gen.go"
	if err := os.WriteFile(outputPath, formatted, 0644); err != nil {
		fmt.Fprintf(os.Stderr, "failed to write decoders file: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("Generated %s with %d decoders\n", outputPath, len(events))
}
